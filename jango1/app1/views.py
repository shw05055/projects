...
def question_create(request):
    ...
    # question form으로 들어올 때
    # 질문을 등록할 때
    # POST 방식인지 GET 방식인지 확인
    # 이유: 단순히 질문등록 페이지를 오픈한것인지
    # 아니면 질문등록 페이지에 데이터를 입력해서 저장한 것인지를 구분
    if request.method == "POST":
        # forms.py에 설정된 QuestionForm 클래스를 호출
        # request.POST: 사용자가 입력한 데이터를 받아온다
        form = QuestionForm(request.POST)
        # 변수 form에 들어온 값이 올바른 값(정상적인 값)인지를 확인한다.
        if form.is_valid():
            # 저장을 하기 전 잠시 유보.
...
# post방식: HTML의 BODY 영역에 데이터를 싫어 보내는 방식(form)
#          대용량의 데이터를 보낼 수 있고, 주소에 실어 보내지 않기 떄문에
#          get방식에 비해 보안성 또한 좋다
...
# <!--템플릿 태그-->
# <!--{% if (요청 받은 값(question_list)%} : question_list가 있을 경우-->
# <!--{% for 임시변수 in 요청받은 값(딕셔너리 키값)%} : 딕셔너리의 키의 value를-->
# <!--반복하여 순차적으로 임시변수에 대입-->
# <!--{{ 임시변수.컬럼 }} : for문에 의해 선언된 임시변수의 컬럼 출력-->
# {{ }}: 데이터와 관련된 내용을 출력할 때 사용하는 템플릿 태그
# {% %}: 템플릿 태그의 속성, 문법등을 사용할 때 쓰는 태그